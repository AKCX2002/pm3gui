...
class LfIoPage extends StatefulWidget {
  const LfIoPage({super.key});

  @override
  State<LfIoPage> createState() => _LfIoPageState();
}

class _LfIoPageState extends State<LfIoPage> {
  String _rawData = '';
  String _lastCmd = '';
  String _result = '';
    ...
    return SplitPageLayout(
      side: Column(
        ...
        children: [
          ActionCard(
              title: '读取',
              subtitle: '读取 ioProx 卡片',
              icon: Icons.nfc,
              onTap: () => _execute(LfIoCmd.reader())),
                      ...
                      Wrap(spacing: 8, runSpacing: 8, children: [
                        ElevatedButton.icon(
                            onPressed: _rawData.isNotEmpty
                                ? () => _execute(LfIoCmd.clone(_rawData))
                                : null,
                            icon: const Icon(Icons.copy, size: 18),
                            label: const Text('克隆')),
                        OutlinedButton.icon(
                            onPressed: _rawData.isNotEmpty
                                ? () => _execute(LfIoCmd.sim(_rawData))
                                : null,
