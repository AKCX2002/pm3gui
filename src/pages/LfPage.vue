  ...
  <el-container style="height: calc(100vh - 40px)">
    <el-aside width="200px" style="border-right: 1px solid var(--el-border-color-light)">
      <el-menu :default-active="activeProto" @select="handleSelect">
        <el-menu-item v-for="p in lfProtocols" :key="p.id" :index="p.id">
          {{ p.label }}
        </el-menu-item>
      </el-menu>
    </el-aside>
...
import { lfProtocols } from "../config/protocols";
import { useTerminalStore } from "../stores/terminal";
import ProtocolPanel from "../components/ProtocolPanel.vue";

const store = useTerminalStore();
const activeProto = ref(lfProtocols[0]?.id ?? "");
const activeProtocol = computed(() => lfProtocols.find(p => p.id === activeProto.value));
